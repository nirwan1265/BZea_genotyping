...
gap_cM <- 0.10   # try 0.02, 0.05, 0.10
gap_bp <- 25000000
...
bridge_donor_runs <- function(seg, donor_set, gap_cM, gap_bp) {
  if (nrow(seg) == 0) return(data.table())
  ...
  while (i <= nrow(seg)) {
    if (!(seg$state[i] %in% donor_set)) { i <- i + 1L; next }

    chr <- seg$Chr[i]
    j <- i

    while (TRUE) {
      # extend through consecutive donor runs
      while (j + 1L <= nrow(seg) &&
             seg$Chr[j+1L] == chr &&
             (seg$state[j+1L] %in% donor_set)) {
        j <- j + 1L
      }

      # candidate gap block after j
      g_start <- j + 1L
      if (g_start > nrow(seg) || seg$Chr[g_start] != chr) break

      # walk forward across non-donor runs to compute gap block size
      g_end <- g_start
      gap_sum_cM <- 0.0
      gap_start_bp <- seg$Start[g_start]
      gap_end_bp <- seg$End[g_start]

      while (g_end <= nrow(seg) &&
             seg$Chr[g_end] == chr &&
             !(seg$state[g_end] %in% donor_set)) {
        gap_sum_cM <- gap_sum_cM + seg$len_cM[g_end]
        gap_end_bp <- seg$End[g_end]
        g_end <- g_end + 1L
        if (gap_sum_cM > gap_cM) break
      }

      gap_span_bp <- as.numeric(gap_end_bp - gap_start_bp + 1L)

      # g_end is first run AFTER the gaps
      if (gap_sum_cM <= gap_cM &&
          gap_span_bp <= gap_bp &&
          g_end <= nrow(seg) &&
          seg$Chr[g_end] == chr &&
          (seg$state[g_end] %in% donor_set)) {
        # bridge: include the gap block + next donor block
        j <- g_end
        next
      } else {
        break
...
for (ii in seq_along(files)) {
  ...
  if ("CHROM" %in% nm) {
    ...
    message("SKIP missing CHROM col: ", basename(f), " cols=", paste(nm, collapse=","))
    next
  }
  ...
  if ("POS" %in% names(dt)) {
    ...
    message("SKIP missing POS col: ", basename(f))
    next
  }
  ...
  if ("STATE" %in% names(dt)) {
    ...
    message("SKIP missing STATE col: ", basename(f))
    next
  }

  # keep only required cols + order
  dt <- dt[, .(CHROM, POS, STATE)]
  dt <- dt[CHROM %in% names(chr_len)]
  setorder(dt, CHROM, POS)
  ...
  for (chr in names(chr_len)) {
    ...
    tr_chr <- bridge_donor_runs(seg, donor_set=donor_set, gap_cM=gap_cM, gap_bp=gap_bp)
